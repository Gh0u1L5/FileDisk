...
{
	Byte High = *PlainText >> 4;
	Byte Low = *PlainText & 0x0F;

	Byte Result = (High ^ Low) & ((Key >> 4) ^ (Key & 0x0F));
	Low = Low ^ Result;
	High = High ^ Result;
	
	*PlainText = (High << 4) | Low;
}
void Decrypt(Byte Key, Byte *CipherText)
{
	Byte High = *CipherText >> 4;
	Byte Low = *CipherText & 0x0F;

	Byte Result = (High ^ Low) & ((Key >> 4) ^ (Key & 0x0F));
	Low = Low ^ Result;
	High = High ^ Result;

	*CipherText = (High << 4) | Low;
}
    ...
    case IOCTL_FILE_DISK_OPEN_FILE:
        {
            ...
            if (io_stack->Parameters.DeviceIoControl.InputBufferLength <
				...
				((POPEN_FILE_INFORMATION)Irp->AssociatedIrp.SystemBuffer)->KeyLength)
            {
                status = STATUS_INVALID_PARAMETER;
                Irp->IoStatus.Information = 0;
                break;
            ...
            {
                device_extension->security_client_context = (PSECURITY_CLIENT_CONTEXT)
    ...
    for (;;)
    {
            ...
            ))
        {
            ...
            case IRP_MJ_READ:
				...
				for(BufferPos = 0; BufferPos < io_stack->Parameters.Read.Length; BufferPos++)
				{
					for(KeyPos = device_extension->key.Length; KeyPos != 0; KeyPos--)
						Decrypt(device_extension->key.Buffer[KeyPos - 1], (Byte *)buffer + BufferPos);
				}
            ...
            case IRP_MJ_WRITE:
				...
				if(buffer != NULL)
				{
					for(BufferPos = 0; BufferPos < io_stack->Parameters.Read.Length; BufferPos++)
					{
						for(KeyPos = 0; KeyPos < device_extension->key.Length; KeyPos++)
							Encrypt(device_extension->key.Buffer[KeyPos], (Byte *)buffer + BufferPos);
					}
				}
                ZwWriteFile(
                    device_extension->file_handle,
    ...
    )
{
	...
	device_extension->key.Length = open_file_information->KeyLength;
    device_extension->key.MaximumLength = open_file_information->KeyLength;
    device_extension->key.Buffer = (PCHAR)ExAllocatePool(NonPagedPool, open_file_information->KeyLength);

	RtlCopyMemory(
        device_extension->key.Buffer,
        open_file_information->FileName + open_file_information->FileNameLength + 1,
        open_file_information->KeyLength
        );
